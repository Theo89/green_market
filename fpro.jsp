<%-- 
    Document   : fpro
    Created on : Sep 4, 2015, 10:16:10 AM
    Author     : TEOM
--%>
...
<%
    ...
    if(request.getParameter("client")!=null){
     id=Integer.parseInt(request.getParameter("client"));
   ...
   if(request.getParameter("client")==null){
     id=(Integer)request.getAttribute("message");
         ...
         request.setAttribute("message", id);
          ...
          RequestDispatcher reqDispatcher = request.getRequestDispatcher("fpro.jsp");} 
...
window.history.forward(1);
                            ...
                            <div id="page">
                                <div id="firstp">
                            <form action="fpro.jsp" method="POST">
                                    <div id="sr">Enter product that you would like to sell and see potential offers: </div><input type="text" name="buy">
                                    <input type="submit" name="v1" value="SEARCH" />
                                        </br>
                                    <input type="hidden" id="thisField" name="client" value="<%= id %>" >
                                    <input type="submit" name="b1" value="SHOW MY PRODUCTS" />
                                    </form>
                                    </div>
                            ...
                            <form action="fpro.jsp" method="POST" >
                             ...
                             <input type="hidden" id="thisField" name="client" value="<%= id %>" >
                           <div id="no"> ENTER NEW PRODUCT TO SELL</div></br>
                            <div id="no1">Enter product that you would like to sell</div>  <input type="text" name="prname"  /> </br>
                           <div id="no1"> Enter location of your fields</div><input type="text" name="location"  /></br>
                          <div id="no1">  Enter price that you would like to sell your product(euros per kilo)</div><input type="text" name="price"  /></br>
...
SELECT * from offer WHERE name="<%= request.getParameter("buy") %>" ;
</sql:query>
<table class="t1">
<tr class="t2">
   <th>Name of product of offer</th>
...
Connection conn = DriverManager.getConnection(
                "jdbc:mysql://localhost:3306/reg", "root", "%so11333");
                 ...
                 statement = conn.prepareStatement("SELECT owner_id FROM offer WHERE offer_id=?");
 ...
 <sql:setDataSource var="snapshot" driver="com.mysql.jdbc.Driver"
     ...
     user="root"  password="%so11333"/>
  <sql:query dataSource="${snapshot}" var="result">
SELECT * from entries WHERE client_id="<%= resultSet.getInt("owner_id") %>" ;
</sql:query>
<table class="t1">
<tr class="t2">
   <th>EMAIL</th> 
...
<%
     if( request.getParameter("location")!=null    && request.getParameter("prname")!=null && request.getParameter("price")!=null && request.getParameter("b3")!=null){
...
<form action="fpro.jsp" method="POST">
<input type="hidden" id="thisField" name="client" value="<%= id %>" >
<div id="del">Enter id of product you would like to withdraw:</div><input type="text" name="with"  />
...
<sql:update dataSource="${snapshot}" var="count">
  DELETE FROM product WHERE item_id = <%= offerd%>;
